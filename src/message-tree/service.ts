  ...
  async say(residentId: string, message: string, windowId: string): Promise<HistoryNode> {
    ...
    const prompt =
      pass !== undefined && pass.contextPrefix.length > 0
        ...
        : message;
    // 真正调用 responder 之前由 SessionRegistry 签发窗级回执。此前这份回执
    // 只存在于注册表单测，生产 say 路径没有接它，因而既不能过滤迟到结果，
    // 也不可能给 MV-B03 留下派发/回执/丢弃三类可审计事件。
    const dispatchReceipt = this.#dispatch?.issueDispatch(windowId);
    ...
    // await 是窗可能死亡或换代的唯一缝。结果回来后先验完整回执，再碰消息树；
    // 迟到结果一字不落树，避免 setHead 最后才抛错而留下孤儿节点。
    if (
      dispatchReceipt !== undefined &&
      this.#dispatch !== undefined &&
      !this.#dispatch.belongsToActiveWindow(dispatchReceipt)
    ) {
